...
**Gemini Embedding** (`gemini-embedding-001`) is Google's state-of-the-art embedding model, achieving top performance on the Massive Multilingual Text Embedding Benchmark (MMTEB). With a 2,048-token context window and up to 3,072 embedding dimensions, it represents the best-in-class solution for text retrieval — capturing full chunk content with rich semantic representations trained on a massive, broad corpus that includes medical and scientific literature. Available via Vertex AI, it integrates naturally with our existing GCP infrastructure.

| | **MedSigLIP (text encoder)** | **all-MiniLM-L6-v2** | **Gemini Embedding** |
|---|---|---|---|
...
| **SigLIP's 64-token context limit** | Regulation chunks (~150–200 tokens) are truncated to ~40–60 words, degrading embedding quality | Mitigated by splitting regulations at 1,000 chars with 120-char overlap so critical terms appear in chunk openings; upgrade path to Gemini Embedding (2,048 tokens) eliminates truncation entirely |
...
- **Embedding model upgrades:** The `vector_store.py` Option A/B pattern allows hot-swapping between MedSigLIP, `all-MiniLM-L6-v2`, and Gemini Embedding. We plan to implement Gemini Embedding (`gemini-embedding-001`) via the Vertex AI API as the production embedding model, leveraging its 2,048-token window and SOTA retrieval quality while staying within our existing GCP infrastructure.
